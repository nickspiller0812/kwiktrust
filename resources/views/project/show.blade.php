...
<div class="row px-5 pt-3" style="">
    <button id="go-back" onclick="window.history.back();" class="btn btn-default back-green px-3">
        ◄
    </button>
...
<div class="align-content-center p-5">
    ...
    <div class="row">
        ...
        @foreach($files as $file)
            <div class="card col-md-2 p-0 m-4 generatedFile" id="generatedFile">
                    ...
                    <ul class="list-unstyled list-inline font-small">
                        <span class="btn-group-sm">
                          {{--<button type="button" class="btn btn-success bmd-btn-fab">--}}
                              @if(\Carbon\Carbon::createFromFormat('d/m/Y', $file->expiration_time) > \Carbon\Carbon::now())
                                    @if(\Carbon\Carbon::createFromFormat('d/m/Y', $file->expiration_time) > \Carbon\Carbon::now()->addMonth())
                                        <button type="button" class="btn btn-success bmd-btn-fab">
                                            <i class="material-icons glyphicon glyphicon-alert"></i>
                                        </button>
                                    @else
                                        <button type="button" class="btn btn-success bmd-btn-fab" style="color:yellow; background-color: yellow;">
                                              <i class="material-icons glyphicon glyphicon-alert"></i>
                                        </button>
                                    @endif
                              @else
                                  <button type="button" class="btn btn-success bmd-btn-fab" style="color:red; background-color: red;">
                                    <i class="material-icons glyphicon glyphicon-alert" ></i>
                                  </button>
                              @endif
                ...
                <div class="modal-content">
                    ...
                    <form class="file-upload" method="POST" action="/file/upload" enctype="multipart/form-data">
                        ...
                        <div class="modal-body px-5">
                            ...
                            <div class="row mt-4">
                                <input type="text" id="fileReleaseDate" name="release_date" class="form-control" placeholder="Release date e.g. 01/01/1970" aria-describedby="inputGroupMaterial-sizing-default">
                            </div>

                            <div class="row mt-4">
                                <input type="text" id="fileExpirationDate" name="expiration_date" class="form-control" placeholder="Expiration date e.g. 01/01/1970" aria-describedby="inputGroupMaterial-sizing-default">
                            </div>
    ...
    <div class="row">
        @foreach($project->folders as $folder)
            <div class="card col-md-2 p-4 text-center m-4">
                ...
                <div class="card-body">
                    <div class="row mb-3">
                        <div class="col-md-12">
                            <img width="120px" src="https://s3-us-west-2.amazonaws.com/s.cdpn.io/14179/circle.png">
                        </div>
                    </div>
                    <div class="row">
                ...
                <a href="/folders/{{$folder->id}}" class="btn btn-default login-green text-white m-auto">
                    go
                </a>
            </div>
        @endforeach
            <button type="button" class="btn btn-primary m-4" data-toggle="modal" style="color:#aaca41 !important;" data-target="#basicExampleModalFolder">
                Add folder
            </button>
    </div>
    ...
    <div class="modal fade" id="basicExampleModalFolder" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">

                <div class="modal-header">
                    <h5 class="modaadd filel-title" id="exampleModalLabel">Add folder</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>

                <form method="POST" action="/folders/create" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <div class="modal-body px-5">
                        <input type="hidden" name="project_id" value="{{$project->id}}">

                        <div class="row mt-4">
                            <input type="text" name="name" class="form-control" placeholder="Folde name" aria-describedby="inputGroupMaterial-sizing-default">
                        </div>
                    </div>

                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-default login-green mr-4">Save changes</button>
                    </div>
                </form>
            </div>
        </div>
